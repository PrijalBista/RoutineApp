...
export default class Tutorial extends Component {
  ...
  static navigationOptions ={
  	drawerIcon:(
  		<Image source={require('../assets/DrawerIcons/tutorial.jpg')}
  ...
  componentDidMount(){
    this.fetchTutorials(false);
  }
  fetchTutorials(pulled){
      if(pulled){
      ...
      .then((responseJson)=>{
        ...
        AsyncStorage.setItem('cachedTutorial', JSON.stringify(responseJson)).done();      
      }).catch((error)=>{
        AsyncStorage.getItem("cachedTutorial").then((data) => {
          if(data!==null){
  ...
  renderTutorials(){
    if(this.state.isLoading){
      ...
      let labno=[[],[]];
      let labsarr=[];
      var labObj = this.state.data.tutorial;
  ...
  render() {
    return (
      <Container>
            ...
            <Body><Title>Tutorials</Title></Body>
            <Right/>  
          </Header>
      		  <Content
              refreshControl = {
              <RefreshControl
                ...
                onRefresh={this.fetchTutorials.bind(this,true)}
              /> }
            >
